...
def ParseArgs():
    ...
    parser.add_argument('--data', default='DrugBank', type=str, help='DrugBank DGIdb LINCS  name of dataset')
    ...
    parser.add_argument('--use_joint_encoding', type=lambda v: str(v).lower() in {'1', 'true', 'yes'}, default=None, # 联合编码（未指定时按数据集自动设置）
    ...
    # 联合编码路径（use_joint_encoding=True 时使用）
    ...
    if args.one_hop_weight is None:
        args.one_hop_weight = 3.5 if args.data == 'DGIdb' else 1.5
    if args.two_hop_weight is None:
        args.two_hop_weight = 2.5
