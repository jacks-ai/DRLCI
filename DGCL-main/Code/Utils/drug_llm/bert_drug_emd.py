...
def main(args):
    ...
    with torch.no_grad():
        for i in tqdm(range(0, len(all_texts), batch_size), desc="生成嵌入"):
            ...
            # 使用Mean Pooling（排除Padding，只对有效token求平均）
            ...
            for drug_id, embedding in zip(batch_drug_ids, mean_embeddings):
...
def build_default_args():
    ...
    # 添加bert前缀和mean后缀
    output_path = drug_text_dir / f"bert_{dataset.lower()}_emd_mean.npy"
